    ...
    if form.vaildate_on_submit():
        ...
        if nickname_user:
            if nickname_user.nickname == form.nickname.data:
                form.nickname.errors.append('이미 등록된 닉네임입니다.')
        if form.email.errors or form.nickname.errors:
            return render_template('users/signup.html', form=form)

        user = User(email=form.email.data, nickname=form.nickname.data, age=form.age.data, password=SHA256.encrypt(form.password.data)) 
        db.session.add(user)
        db.session.commit()
...
def signin():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            form.email.errors.append('가입하지 않은 이메일입니다.')
